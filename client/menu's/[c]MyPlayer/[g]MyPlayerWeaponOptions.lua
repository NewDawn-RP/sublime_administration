...
function _Admin.Panel:MyPlayerWeaponOptions(rank)
    ...
    --         _Admin.SendServerLogs("[".._.rank.name.." - "..GetPlayerName(PlayerId()).."] S'est give toutes les armes")
    --     end
    -- });
    ...
    for k,v in pairs(_Admin.Weapon.List) do
        RageUI.Button(v.label, nil, {RightLabel = "~c~→→→"}, true, {
            onSelected = function()
                local qty = KI("Quantité ~o~souhaité~s~", "", 15)
                qty = tonumber(qty)
                if type(qty) == 'number' then
                    TriggerServerEvent(_Admin.Prefix.."GiveWeapon", _.rank, v.name, qty)
                    --_Admin.SendServerLogs("[".._.rank.name.." - "..GetPlayerName(PlayerId()).."] S'est give "..ESX.GetWeaponLabel(v.name).." avec "..qty.." munitions")
                    _Admin.SendServerLogs("[".._.rank.name.." - "..GetPlayerName(PlayerId()).."] S'est give "..v.label.." avec "..qty.." munitions")
                else
...
--                         TriggerServerEvent(_Admin.Prefix.."GiveWeapon", _.rank, v.name, qty)
--                         --_Admin.SendServerLogs("[".._.rank.name.." - "..GetPlayerName(PlayerId()).."] S'est give "..ESX.GetWeaponLabel(v.name).." avec "..qty.." munitions")
--                         _Admin.SendServerLogs("[".._.rank.name.." - "..GetPlayerName(PlayerId()).."] S'est give "..ESX.GetWeaponLabel(v.name))
--                     else
--                         ESX.ShowNotification("~r~Quantité invalide")
--                     end 
--                 end
